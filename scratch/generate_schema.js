...
// 4. Workflow Tables (Extract from workflow_schema.ts but ONLY up to documentTags!)
let workflowSchema = fs.readFileSync('scratch/workflow_schema.ts', 'utf8');
if (workflowSchema.includes('export const documentTags')) {
    workflowSchema = workflowSchema.substring(0, workflowSchema.indexOf('export const documentTags'));
}
content += '\n' + workflowSchema;

// 5. Save
fs.writeFileSync('packages/db/src/schema/index.ts', content, 'utf8');
console.log('Generated index.ts successfully');
